...
(() => {
  ...
  const deepClone = (value) => {
    ...
    try {
      ...
      console.warn("Sell macro | Failed to clone value", error, value);
      return value;
    }
  };
  ...
  const getStoredState = (actor) => {
    if (!actor?.getFlag) return {};
    try {
      const scope = actor.getFlag(MODULE_ID, FLAG_SCOPE);
      if (!scope || typeof scope !== "object") return {};
      const stored = scope?.[FLAG_KEY];
      if (!stored || typeof stored !== "object") return {};
      return deepClone(stored) ?? {};
    } catch (error) {
      console.warn("Sell macro | Failed to read stored state", error);
      return {};
    }
  };

  const persistState = async (actor, updates) => {
    if (!actor?.setFlag) return;
    try {
      const scope = actor.getFlag(MODULE_ID, FLAG_SCOPE);
      const clonedScope = scope && typeof scope === "object" ? deepClone(scope) : {};
      const current = clonedScope?.[FLAG_KEY] && typeof clonedScope[FLAG_KEY] === "object" ? clonedScope[FLAG_KEY] : {};
      clonedScope[FLAG_KEY] = { ...current, ...updates };
      await actor.setFlag(MODULE_ID, FLAG_SCOPE, clonedScope);
    } catch (error) {
      console.warn("Sell macro | Failed to persist state", error, updates);
    }
  };

  const performSale = async (actor, percentage) => {
    const state = getStoredState(actor);
    const salePercentage = Math.max(0, Math.min(100, Number.isFinite(Number(percentage)) ? Number(percentage) : DEFAULT_SELL_PERCENTAGE));
    ...
    if (state?.treasureItemId) {
      existingTreasure = treasureItems.find((item) => item.id === state.treasureItemId) ?? null;
    }

    if (!existingTreasure && sellFolderId) {
    ...
    const baseFactor = salePercentage / 100;
    let copperTotal = 0;
    ...
    let totalCopperAfterSale = existingTreasure ? coinsToCopper(existingTreasure.system?.price?.value) : 0;
    ...
    await persistState(actor, {
      percentage: salePercentage,
      treasureItemId: saleTreasureItem?.id ?? existingTreasure?.id ?? null,
      proceedsName: saleTreasureItemName,
      lastSaleCopper: totalCopperAfterSale || copperEarned,
      lastSoldAt: Date.now(),
    });
  };
  ...
  const openSellDialog = (actor) => {
    const state = getStoredState(actor);
    const defaultPercentage = Number.isFinite(Number(state?.percentage)) ? Number(state.percentage) : DEFAULT_SELL_PERCENTAGE;
    const content = `
      <form>
        <div class="form-group">
          <label>${game.i18n.localize?.("PF2E.SellPercentage") ?? "Sell percentage"}</label>
          <input type="number" name="sell-percentage" value="${defaultPercentage}" min="0" max="100" step="1" />
    ...
    const dialog = new Dialog({
      ...
      buttons: {
        ...
        sell: {
          ...
          callback: (html) => {
            ...
            const percent = Number(input?.value ?? defaultPercentage);
      ...
      close: () => {
        const input = dialog.element?.find?.('input[name="sell-percentage"]')?.[0];
        if (!input) return;
        const value = Number(input.value ?? defaultPercentage);
        if (Number.isFinite(value)) {
          persistState(actor, { percentage: Math.max(0, Math.min(100, value)) }).catch((error) => {
            console.warn("Sell macro | Failed to save percentage on close", error);
          });
        }
      },
    });
